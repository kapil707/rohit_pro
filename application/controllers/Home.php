...
class Home extends CI_Controller {
	...
	public function index()
	{
		...
		$where1 = array('status'=>'1');
		$data["tbl_slider2"] = $this->Main_Model->get_single_data_result("tbl_slider2",$where1,"id","asc");
		
		$where1 = array('status'=>'1','show_in_home'=>'1');
		$data["tbl_media"] = $this->Main_Model->get_single_data_result("tbl_media",$where1,"id","asc");
		
		$where1 = array('status'=>'1');
		$data["tbl_icon1"] = $this->Main_Model->get_single_data_result("tbl_icon1",$where1,"id","asc");
		
		$where1 = array('status'=>'1');
		$data["tbl_icon2"] = $this->Main_Model->get_single_data_result("tbl_icon2",$where1,"id","asc");
		
		$data["home_dt1"] = $this->db->query("select * from tbl_home where id='1'")->row();
		$data["home_dt2"] = $this->db->query("select * from tbl_home where id='2'")->row();
		$data["tbl_toptext"] = $this->db->query("select * from tbl_toptext")->result();
	...
	public function lead()
	{
		...
		{ 
			...
			$row = $this->db->query("select * from tbl_leads where date='$date' and system_ip='$system_ip'")->row();
			if($row->id=="")
			{
